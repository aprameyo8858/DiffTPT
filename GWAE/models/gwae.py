...
from models.iwrasgw_pytorch import iwrasgw_gpu
...
class GromovWassersteinAutoEncoder(VAE):
    ...
    def step_batch(self, batch: tuple[torch.Tensor, torch.Tensor], optimizers=None, progress=None, training=False):
        ...
        psnr = self.calculate_psnr(x, x2)
        #print(psnr,"is the PSNR")

        zs = self.sample_prior(batch_size)
        x2s = self.dec_block(zs)
        
        device = 'cuda'
        # scale_z = zs.std()        #this was commented out
        ...
        x2s_=x2s
        x2s_=x2s_.view(x2s_.shape[0],-1)
        #x2s_=x2s_.t()
        #print(x2s_.shape,zs_.shape)
        # GW(data distro, model prior)
        #loss_gw=sgw_gpu_original(x2s_.to('cuda'),zs_.to('cuda') ,'cuda',nproj=500,tolog=False,P=None)
        #loss_gw = DSGW(x2s_.to('cuda') , zs_.to('cuda') , L=5, kappa=10, p=2, s_lr=0.1, n_lr=10, device='cuda', nproj=500)
        #loss_gw = RPSGW(x2s_.to('cuda') , zs_.to('cuda') , L=10  , nproj=500, p=2, kappa=50 )
        #loss_gw = MaxSGW(x2s_.to('cuda') , zs_.to('cuda') , p=2, s_lr=0.1, n_lr=10, device='cuda', adam=False, nproj=500)
        #loss_gw = ebrpsgw_gpu(x2s_.to('cuda') , zs_.to('cuda'),  L=10  , nproj=500, p=2, kappa=50 )
        loss_gw = EBSGW(x2s_.to('cuda') , zs_.to('cuda') , L=10, p=2, device='cuda')
        #loss_gw = rasgw_gpu(x2s_.to('cuda'), zs_.to('cuda') , 'cuda', nproj=500,tolog=False,P=None)
        #loss_gw = iwrasgw_gpu(x2s_.to('cuda'), zs_.to('cuda') , 'cuda', nproj=500,tolog=False,P=None)
        ...
        #print(loss_gw/loss_gw_)
        # W(data, model)
        loss_gw = max(0, loss_gw)
        #loss_gw = loss_gw*10000
