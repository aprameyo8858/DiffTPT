...
def sink_(xs, xt, device, nproj=200, P=None, kappa=50):
    ...
    if P is None:
        L=nproj
        indices_X = torch.randint(low=0, high=xs2.shape[0], size=(L,), device=device)
        indices_Y = torch.randint(low=0, high=xt2.shape[0], size=(L,), device=device)

        #linear_projection = nn.Linear(Y.shape[1], X.shape[1]).to(device)
        #Y = Y.to(device)
        # Apply the linear transformation
        #Y_projected = linear_projection(Y)
        theta = (xs2.detach()[indices_X] - xt2.detach()[indices_Y])
        #theta1= (xs2.detach()[np.random.choice(xs2.shape[0], nproj, replace=True)] - xs2.detach()[np.random.choice(xs2.shape[0], nproj, replace=True)])
        #theta2= (xt2.detach()[np.random.choice(xt2.shape[0], nproj, replace=True)] - xt2.detach()[np.random.choice(xs2.shape[0], nproj, replace=True)])
        #theta2 = (xt2.detach()[indices_X] - xt2.detach()[indices_Y])
...
def ebrpsgw_gpu(X, Y, L=10, p=2, device='cuda', kappa=50, nproj=200):
    ...
    xsp, xtp = sink_(X, Y, device, nproj=nproj,kappa=kappa)
    
    # Compute the Gromov-Wasserstein distance between the projections
    
    sgw = gromov_1d(xsp, xtp, tolog=False)
    sgw = weighted_distance(sgw)
    #sgw.append(sw.item())
    #print(sgw)
