...
class Generator(torch.nn.Module):
    def __init__(self, z_dim, output_dim=2):
        ...
        self.z_dim = z_dim
        self.output_dim = output_dim  # Store the output_dim to use it for the final layer

        # Define the layers of the generator
        self.map1 = torch.nn.Linear(self.z_dim, 128, bias=True)
        self.map2 = torch.nn.Linear(128, 128, bias=True)
        self.map3 = torch.nn.Linear(128, 128, bias=True)
        self.map4 = torch.nn.Linear(128, self.output_dim, bias=True)  # Use output_dim here
...
class Generator_base(torch.nn.Module):
    def __init__(self):
        super(Generator, self).__init__()
